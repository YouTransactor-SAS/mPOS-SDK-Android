...
package com.youtransactor.sampleapp.transactionView.steps.display_payment_result;
...
public class DefaultDisplayPaymentResultStep implements DisplayPaymentResultStep {

    private final TransactionViewBase transactionViewBase;

    public DefaultDisplayPaymentResultStep(TransactionViewBase transactionViewBase) {
        this.transactionViewBase = transactionViewBase;
    }

    @Override
    public void execute(EventCommand genericCommand) {
        final EventDspPaymentRslt eventCmd = (EventDspPaymentRslt) genericCommand;

        final DisplayMsgFragment.Params params = new DisplayMsgFragment.Params(
                eventCmd.getResult(),
                eventCmd.getMessage(),
                eventCmd.getResultTag()
        );
